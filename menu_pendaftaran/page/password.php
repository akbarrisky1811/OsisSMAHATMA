<?php
  if (isset($_POST["submit-password"])) {
    if (!empty($_POST["passl"])&&!empty($_POST["passb"])&&!empty($_POST["passk"])) {
      ...
      //get password 
      $query = mysqli_query($koneksi, "SELECT `password` FROM `user_pendaf` WHERE `id_user`='$id_user'"); 
      ...
      if (empty($pl)&&empty($pb)&&empty($pk)) {
        ...
        $password = $passb;
        mysqli_query($koneksi,"UPDATE `user_pendaf` SET `password`='$password' WHERE `id_user`='$id_user'");
        $notif="editberhasil";
      }
    }
  }
