  ...
  if (isset($_POST["submit-profil"])) {
    if (isset($_POST['username'])&&isset($_POST['nama'])) {
      ...
      if ($username_baru!=$username) {
      	if (mysqli_num_rows(mysqli_query($koneksi, "SELECT `username` FROM `user_pendaf` WHERE `username` = '$username_baru'"))==0) {
      		mysqli_query($koneksi,"UPDATE `user_pendaf` SET `username`='$username_baru', `nama`='$nama_baru' WHERE `id_user`='$id_user'");
  ...
  if (isset($_POST["submit-foto"])) {
    if (isset($_FILES['foto'])) {
      ...
      if(move_uploaded_file($file_tmp,$direktori)){ 
        mysqli_query($koneksi,"UPDATE `user_pendaf` SET `foto`='$nama_file' WHERE `id_user`='$id_user'");
